...
def test_quarantine_creates_reports(app_config, tmp_path):
    ...
    record = quarantine.isolate(archive, reason="contains executable", source="desktop", indicators=indicators)
    assert Path(record.quarantined_path).exists()
    report_dir = Path(app_config.reports_root) / "quarantine"
    json_reports = list(report_dir.glob("quarantine-*.json"))
    html_reports = list(report_dir.glob("quarantine-*.html"))
    assert json_reports and html_reports
    payload = json_reports[-1].read_text(encoding="utf-8")
