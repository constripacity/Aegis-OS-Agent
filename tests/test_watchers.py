...
from aegis.core.bus import ClipboardEvent, EventBus, FileSystemEvent
...
def test_clipboard_watcher_publishes_events(app_config) -> None:
    ...
    def collect_clipboard(event: object) -> None:
        assert isinstance(event, ClipboardEvent)
        events.append(event.content)

    bus.subscribe("clipboard", collect_clipboard)
...
def test_directory_watcher_scan(tmp_path, app_config) -> None:
    ...
    def collect_fs(event: object) -> None:
        assert isinstance(event, FileSystemEvent)
        events.append(event.path)

    bus.subscribe("filesystem", collect_fs)
